...
module.exports = async (argv) => {
  program
    ...
    .option(
      "-o, --output <path>",
      "relative path to store output media (image, image pattern, gif, or mp4)",
      "out.png"
    )
    .option("-w, --width <number>", "optional output width", (s) => parseInt(s))
    .option("-h, --height <number>", "optional output height", (s) =>
      parseInt(s)
    )
    .option(
      "-b, --background <css-color-value>",
      "optional output background color",
      "transparent"
    )
    .option("-a, --args <value>", "the list of Chromium flags", (s) =>
      s.split(" ")
    )
